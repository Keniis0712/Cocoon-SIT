...
def test_delete_chat_group_cleans_plugin_dispatch_records_for_wakeups(client, auth_headers):
    ...
    create_response = client.post(
        "/api/v1/chat-groups",
        headers=auth_headers,
        json={"name": "Cleanup Group Plugin Dispatch", "character_id": character_id, "selected_model_id": model_id},
    )
    assert create_response.status_code == 200, create_response.text
    room_id = create_response.json()["id"]
...
def test_delete_chat_group_cleans_plugin_im_delivery_outbox_records(client, auth_headers):
    ...
    create_response = client.post(
        "/api/v1/chat-groups",
        headers=auth_headers,
        json={"name": "Cleanup Group IM Outbox", "character_id": character_id, "selected_model_id": model_id},
    )
    assert create_response.status_code == 200, create_response.text
    room_id = create_response.json()["id"]
    ...
    with container.session_factory() as session:
        ...
        message = Message(chat_group_id=room_id, action_id=action.id, role="assistant", content="Delete room outbox")
        session.add(message)
        session.flush()
