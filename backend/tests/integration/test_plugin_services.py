...
def test_im_plugin_bridges_inbound_chat_and_outbound_reply(client, auth_headers, default_cocoon_id, worker_runtime):
    response = _install_response(
        client,
        auth_headers,
        manifest={
...
async def run(ctx):
    ...
    async def handle_reply(reply):
        ...
        with path.open("a", encoding="utf-8") as fh:
            fh.write(json.dumps({"delivery_id": reply.delivery_id, "reply_text": reply.reply_text}, ensure_ascii=False) + "\\n")
        return ImDeliveryResult(ok=True)

    route = ImInboundRoute(target_type="cocoon", target_id=str(ctx.plugin_config["target_cocoon_id"]))
...
""",
    ...
    while time.time() < deadline:
        ...
        with container.session_factory() as session:
            actions = list(
                session.scalars(
                    select(ActionDispatch).where(ActionDispatch.cocoon_id == default_cocoon_id).order_by(ActionDispatch.created_at.asc())
                ).all()
            )
            user_messages = list(
                session.scalars(
                    select(Message).where(Message.cocoon_id == default_cocoon_id, Message.role == "user").order_by(Message.created_at.asc())
                ).all()
            )
            ...
            if delivered_path.exists():
                delivered_payload = [json.loads(line) for line in delivered_path.read_text(encoding="utf-8").splitlines() if line]
            if (
                len(actions) == 1
...
def test_dependency_builder_archives_packages_with_package_level_dedup(tmp_path):
    ...
    assert json.loads((package_one_root / ".package-info.json").read_text(encoding="utf-8"))["size_bytes"] > 0
