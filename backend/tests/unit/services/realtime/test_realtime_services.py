...
def test_connection_manager_binds_loop_and_removes_stale_websockets():
    import asyncio

    manager = ConnectionManager()
    stale_socket = _DummyWebSocket()

    async def failing_send_json(payload: dict) -> None:
        raise RuntimeError("socket closed")

    stale_socket.send_json = failing_send_json  # type: ignore[method-assign]
    loop = asyncio.new_event_loop()
    try:
        manager.bind_loop(loop)
        assert manager.loop is loop
        asyncio.run(manager.connect("cocoon-2", stale_socket))

        asyncio.run(manager.broadcast_local("cocoon-2", {"type": "ping"}))

        assert "cocoon-2" not in manager.connections
    finally:
        loop.close()
