...
import { Plus, Ticket, Trash2, UserRound } from "lucide-react";
...
import { createInviteCode, deleteInviteCode, getMyInviteSummary, listInviteCodes } from "@/api/invites";
import type { AdminUserRead, GroupRead, InviteCodeCreatePayload, InviteCodeRead, InviteSummary } from "@/api/types/access";
...
export default function InvitesPage() {
  ...
  async function bootstrap() {
    if (!userInfo) return;
    try {
      const [usersResponse, groupsResponse, codesResponse, personalQuota] = await Promise.all([
      ...
      const defaultGroupId = groupsResponse.items.find((item) => item.parent_group_id)?.gid || groupsResponse.items[0]?.gid || "";
      setCodeForm((prev) => ({
        ...prev,
  ...
  return (
    <PageFrame
      ...
      description={t("invites.codesDescription")}
      actions={
    ...
    >
      <div className="mb-6 grid gap-4 xl:grid-cols-3">
        <MetricPanel label={t("invites.personalQuota")} value={renderSummaryValue(t, personalSummary)} hint={userInfo?.username || userInfo?.uid || "-"} />
        <MetricPanel label={`${t("invites.codesTitle")} / ${t("invites.status.active")}`} value={String(activeCodeCount)} />
        <MetricPanel label={`${t("invites.codesTitle")} / ${t("invites.status.used")}`} value={String(usedCodeCount)} />
      </div>
              ...
              return (
                <div key={invite.code} className="rounded-[24px] border border-border/70 bg-background/30 p-4 text-sm">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1 text-xs text-muted-foreground">
                        {(userLabelMap.get(invite.created_by_uid) || invite.created_by_uid || "-")} {"->"} {(userLabelMap.get(invite.parent_uid) || invite.parent_uid || "-")}
                      </div>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div className="mt-3 grid gap-2 text-xs text-muted-foreground md:grid-cols-2">
                    <div>{t("invites.registrationGroup")}: {invite.registration_group_id ? groupLabelMap.get(invite.registration_group_id) || invite.registration_group_id : "-"}</div>
                    <div>{t("invites.sourceId")}: {invite.source_id ? (invite.source_type === "USER" ? userLabelMap.get(invite.source_id) || invite.source_id : invite.source_type === "GROUP" ? groupLabelMap.get(invite.source_id) || invite.source_id : invite.source_id) : "-"}</div>
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setCodeForm((prev) => ({
                      ...
                      source_id: value === "USER" ? userInfo?.uid || "" : value === "GROUP" ? prev.registration_group_id : "",
                    }))
                  }
                >
