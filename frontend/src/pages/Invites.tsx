...
function renderSummaryValue(t: (key: string, options?: Record<string, unknown>) => string, summary: InviteSummary | null) {
  if (!summary) return "0";
...
export default function InvitesPage() {
  ...
  async function bootstrap() {
    ...
    try {
      ...
      const defaultGroupId = selectedGroupId || groupsResponse.items[0]?.gid || "";
      setSelectedGroupId(defaultGroupId);
      setCodeForm((prev) => ({
        ...prev,
        created_for_uid: prev.created_for_uid || userInfo.uid,
        source_id: prev.source_type === "USER" ? prev.source_id || userInfo.uid : prev.source_id,
      }));
      setGrantForm((prev) => ({
        ...prev,
        ...
        <Card className="border-border/70 bg-card/90">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="rounded-2xl border border-border/70 p-4">
              ...
              {selectedGroup ? <div className="mt-2 text-xs text-muted-foreground">{selectedGroup.name}</div> : null}
            </div>
          </CardContent>
        </Card>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="grid gap-2">
                ...
                {codeForm.source_type === "GROUP" ? (
                  ...
                  <Select value={codeForm.source_id || userInfo?.uid || ""} onValueChange={(value) => setCodeForm((prev) => ({ ...prev, source_id: value }))}>
                    <SelectTrigger><SelectValue placeholder={t("invites.selectUser")} /></SelectTrigger>
                    <SelectContent>
                      {users.map((item) => (
                        <SelectItem key={item.uid} value={item.uid}>
                          {item.username} · {item.uid}
